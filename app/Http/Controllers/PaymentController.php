...
class PaymentController extends Controller
{
    public function index(Request $request)
    {
        ...
        // DEBUGGING: Check total payments in database
        $totalPaymentsInDb = Payment::count();
        Log::info('Total payments in database: ' . $totalPaymentsInDb);
        
        $query = Payment::with(['debitNote.paymentPlan.schedule.policy.client']);

        // Filter by client_id if provided
        if ($request->has('client_id') && $request->client_id) {
            Log::info('Filtering by client_id: ' . $request->client_id);
        ...
        if ($request->has('debit_note_id') && $request->debit_note_id) {
            Log::info('Filtering by debit_note_id: ' . $request->debit_note_id);
        ...
        if ($request->has('mode_of_payment_id') && $request->mode_of_payment_id) {
            Log::info('Filtering by mode_of_payment_id: ' . $request->mode_of_payment_id);
        ...
        if ($request->filled('policy_id')) {
            Log::info('Filtering by policy_id: ' . $request->policy_id);
        ...
        if ($request->has('date_from') && $request->date_from) {
            Log::info('Filtering date_from: ' . $request->date_from);
            $query->whereDate('paid_on', '>=', $request->date_from);
        }
        if ($request->has('date_to') && $request->date_to) {
            Log::info('Filtering date_to: ' . $request->date_to);
            $query->whereDate('paid_on', '<=', $request->date_to);
        }

        // DEBUGGING: Get SQL query
        $sql = $query->toSql();
        ...
        // DEBUGGING: Log payment count after pagination
        Log::info('Payments after pagination: ' . $payments->count());
        Log::info('Total payments (with pagination): ' . $payments->total());
        Log::info('Current page: ' . $payments->currentPage());
        Log::info('Last page: ' . $payments->lastPage());
        ...
        if ($request->has('client_id') && $request->client_id) {
            ...
            if ($client) {
                Log::info('Client found: ' . $client->client_name);
            } else {
                Log::warning('Client not found for ID: ' . $request->client_id);
            }
        }

        // DEBUGGING: Log first payment if exists
        if ($payments->count() > 0) {
            $firstPayment = $payments->first();
            Log::info('First payment: ' . json_encode($firstPayment->toArray()));
        } else {
            Log::warning('No payments found to display!');
        }
