...
// ============================================================================
// FIXED VERSION - openClientDetailsModal (Actually Page View)
// ============================================================================
...
async function openClientDetailsModal(clientId) {
  ...
  try {
    ...
    // STEP 3: Update page title (with null checks)
    const clientName = `${client.first_name || ''} ${client.surname || ''}`.trim() || 'Unknown';
    const clientPageName = document.getElementById('clientPageName');
    const clientPageTitle = document.getElementById('clientPageTitle');
    
    if (clientPageName) {
      clientPageName.textContent = clientName;
      console.log('✅ Page name updated');
    } else {
      console.warn('⚠️ clientPageName element not found');
    }
    ...
    if (clientPageTitle) {
      ...
      console.warn('⚠️ clientPageTitle element not found');
    }

    // STEP 4: Populate details (with null check)
    console.log('Populating client details...');
    if (typeof populateClientDetailsModal === 'function') {
      populateClientDetailsModal(client);
      console.log('✅ Details populated');
    } else {
      console.error('❌ populateClientDetailsModal function not found');
    ...
    const clientPageView = document.getElementById('clientPageView');
    if (!clientPageView) {
      console.error('❌ clientPageView element not found');
      throw new Error('clientPageView element is missing from the page');
    }
    
    clientPageView.classList.add('show');
    clientPageView.style.display = 'block';
    console.log('✅ Page view shown');
    
    // STEP 7: Show details content, hide form content (with null checks)
    const clientDetailsPageContent = document.getElementById('clientDetailsPageContent');
    if (clientDetailsPageContent) {
      clientDetailsPageContent.style.display = 'block';
      console.log('✅ Details content shown');
    } else {
      console.warn('⚠️ clientDetailsPageContent element not found');
    }
    
    const clientFormPageContent = document.getElementById('clientFormPageContent');
    if (clientFormPageContent) {
      clientFormPageContent.style.display = 'none';
      console.log('✅ Form content hidden');
    } else {
      console.warn('⚠️ clientFormPageContent element not found');
    }
    
    // STEP 8: Show edit button (with null check)
    const editClientFromPageBtn = document.getElementById('editClientFromPageBtn');
    if (editClientFromPageBtn) {
      editClientFromPageBtn.style.display = 'inline-block';
      console.log('✅ Edit button shown');
    } else {
      console.warn('⚠️ editClientFromPageBtn element not found');
    }

    // STEP 9: Show/hide BOs button based on client_type (with function check)
    if (typeof updateBOsButtonVisibility === 'function') {
      updateBOsButtonVisibility(client);
      console.log('✅ BOs button visibility updated');
    } else {
      console.warn('⚠️ updateBOsButtonVisibility function not found');
    }

    // STEP 10: Setup nav tab listeners for page view
    const navTabs = document.querySelectorAll('#clientPageView .nav-tab');
    if (navTabs.length > 0) {
      navTabs.forEach(tab => {
        // Remove existing listeners by cloning
        const newTab = tab.cloneNode(true);
      ...
      console.log('✅ Nav tabs configured:', navTabs.length);
    } else {
      console.warn('⚠️ No nav tabs found in clientPageView');
    }
...
console.log('✅ Fixed openClientDetailsModal function loaded');
...
// ============================================================================
/* ============================================
   CLIENT MODAL JAVASCRIPT - Add to clients-index.js
   ============================================ */
...
// Toggle Individual/Business fields
function toggleClientFields() {
    ...
    document.querySelectorAll('[data-field-type="individual"]').forEach(el => {
        el.style.display = isIndividual ? '' : 'none';
    });
    
    document.querySelectorAll('[data-field-type="business"]').forEach(el => {
        el.style.display = isIndividual ? 'none' : '';
    });
}
...
//     // Clear any previous errors/styles
//     if(modalForm) {
//         modalForm.querySelectorAll('.input-error').forEach(el => el.classList.remove('input-error'));
//     }
...
//         if(modalForm) {
//             modalForm.reset();
//             modalForm.action = clientsStoreRoute; // Ensure this route variable exists
//         }
//         if(formMethod) formMethod.innerHTML = ''; 
//         if(deleteBtn) deleteBtn.style.display = 'none';
        
//         // Default to Individual
//         if(clientTypeSelect) clientTypeSelect.value = 'Individual';
        
//         // Reset previews
//         const docList = document.getElementById('editClientDocumentsList');
//         if(docList) docList.innerHTML = '';
        
//         // Reset specific UI elements
//         const dobAge = document.getElementById('dob_age');
//         if(dobAge) dobAge.value = '';

//         // --- ERROR FIX START ---
//         // Hum check kar rahe hain ki element exist karta hai ya nahi
//         const imageInput = document.getElementById('image');
//         if (imageInput) {
...
//     } else {
//         // EDIT MODE
//         if(modalForm) modalForm.action = `/clients/${client.id}`;
//         if(formMethod) formMethod.innerHTML = `@method('PUT')`;
//         if(deleteBtn) deleteBtn.style.display = 'inline-block';
//         currentClientId = client.id;

//         // Populate Fields
//         populateFormFields(modalForm, client);
        
//         // Trigger calculations
//         if(client.dob_dor) calculateAge(client.dob_dor);
//     }
...
//     // Also close the page view if it's open
//     if(typeof closeClientPageView === 'function') closeClientPageView();
// }

// /**
//  * Toggles visibility of fields based on 'data-field-type' attribute in HTML.
//  * Used for the new Grid Layout.
//  */
// function toggleClientFields() {
//     const clientTypeSelect = document.getElementById('client_type');
//     if (!clientTypeSelect) return;
...
//     // Select all container divs that have the data attribute
//     const allFields = document.querySelectorAll('[data-field-type]');
...
//                 field.style.display = 'flex'; // Use flex because .form-item is column-flex
//             } else {
//                 field.style.display = 'none';
//             }
//         } else {
//             // Show Business fields (Business or Company)
//             if (fieldType === 'business') {
//                 field.style.display = 'flex';
//                 // Business name usually spans 2 columns in the grid
//                 if (field.querySelector('#business_name')) {
...
// /**
//  * Populates form inputs with client data
//  */
// function populateFormFields(form, client) {
//     // List of all field names in your form
//     const fieldNames = [
//         'first_name', 'surname', 'other_names', 'salutation', 
//         'client_type', 'nin_bcrn', 'dob_dor', 'id_expiry_date', 
//         'passport_no', 'mobile_no', 'alternate_no', 'email_address', 
//         'occupation', 'employer', 'income_source', 'monthly_income', 
//         'source', 'source_name', 'agent', 'agency', 'status', 
//         'signed_up', 'location', 'district', 'island', 'country', 
//         'po_box_no', 'spouses_name', 'pep_comment', 'notes', 'business_name'
//     ];
...
//         if (cb) cb.checked = (client[id] == 1 || client[id] === true);
//     });
    
//     // Handle Image Preview if needed
//     if(client.image) {
//         // You can add logic here to show existing image thumbnail
//     }
// }

// function calculateAge(dobString) {
//     const ageField = document.getElementById('dob_age');
//     if (!ageField || !dobString) return;
